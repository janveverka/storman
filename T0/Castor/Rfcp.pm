...
sub start_wheel {
        ...
        and $file->{target} =~ m/^\/castor/ )
    {
        ...
        $task = POE::Wheel::Run->new(
            Program => sub {
                my $exitcode = -1;
                my @args = ( "nstouch", $file->{target} );
                $exitcode = system(@args);
                if ( $exitcode == 0 ) {
                    @args = (
                        "nssetchecksum", "-n", "adler32", "-k",
    ...
    if ( exists $file->{timeout} ) {
        $file->{alarm_id} =
          $kernel->delay_set( 'monitor_task', $file->{timeout},
            ( $task->ID, 0 ) );
    }
}
    ...
    if ( exists $heap->{task}->{$task_id} ) {
        ...
        if ( $status != 0 ) {
            ...
            if ( !$file->{checked_source} ) {
                $heap->{Self}->Quiet(
                    "Checking if file " . $file->{source} . " exists\n" );
                my $file_status =
                  T0::Castor::RfstatHelper::checkFileExists( $file->{source} );
                ...
                # If it exists continue with the cleanup.
                    ...
                    $heap->{Self}
                      ->Quiet( "Source file " . $file->{source} . " exists\n" );
                    $kernel->yield( 'check_target_exists',
                        ( $task_id, $status ) );
                }
            }
            else {
...
sub rfcp_retry_handler {
    ...
    if (   defined( $heap->{delete_bad_files} )
        && $heap->{delete_bad_files} == 1
        && $deleteTargetFile == 1 )
    {
...
# After creating the dir we retry without waiting and without decreasing retries
    ...
    # Retrying
        ...
        $heap->{Self}
          ->Quiet( "Retry count at " . $file->{retries} . " , retrying\n" );
        $file->{retries}--;

        if ( exists $file->{retry_backoff} ) {
            $heap->{wheel_count}++;
            $kernel->delay_set( 'start_wheel', $file->{retry_backoff},
                ($file) );
        }
        else {
            $heap->{wheel_count}++;
    ...
    # No more retries
        ...
        $heap->{Self}
          ->Quiet( "Retry count at " . $file->{retries} . " , abandoning\n" );
    }
    $kernel->yield( 'wheel_cleanup', ( $task_id, $status ) );
}
