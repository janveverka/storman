...
sub _init {
    ...
    defined( $self->{Receiver}->{Port} ) && defined( $self->{Receiver}->{Host} )
...
sub ReadConfig {
    ...
    T0::Util::ReadConfig($self);
        $self->{Receiver}->{$_} ||= $Logger::Receiver{$_}
    for keys %Logger::Receiver;
}
...
sub Send {
    ...
    if ( $self->{Session} ) {
        if ( scalar @_ == 1 ) {
            ...
            else { $poe_kernel->post( $self->{Session}, 'send', \$ref ); }
        }
        else { $poe_kernel->post( $self->{Session}, 'send', \@_ ); }
        return;
    }
    ...
    if ( scalar @_ == 1 ) {
        if   ( ref($ref) ) { push @{ $self->{Queue} }, $ref; }
        else               { push @{ $self->{Queue} }, \$ref; }
    }
    else { push @{ $self->{Queue} }, \@_; }
}
