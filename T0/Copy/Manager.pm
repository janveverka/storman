...
sub process_file {
    ...
    my $priority = 99;
    if ( $work->{STREAM} ) {
        if ( exists $self->{HighPrioStream}
            && $work->{STREAM} =~ /$self->{HighPrioStream}/ )
        {
            $priority = 33;
        }
        elsif ( exists $self->{NormPrioStream}
            && $work->{STREAM} =~ /$self->{NormPrioStream}/ )
        {
            $priority = 66;
        }
    }
...
sub RemoveClient {
    ...
    if ( !$remainingWorkers ) {
        ...
        if ( $self->{hostnamesQueue}->{$hostname} && my $count =
            $self->{hostnamesQueue}->{$hostname}->get_item_count() )
        {
...
sub send_client_work {
    ...
    my ( $priority, $id, $work ) = $self->ClientQueue($client)->dequeue_next();
    if ( defined $id ) {
...
sub send_work {
    ...
    if ( $self->{State} eq 'Running' ) {
        ...
        while ( ( $priority, $id, $work ) =
            $self->HostnameQueue($hostname)->dequeue_next() )
        {
            if ( defined $id and defined $work ) {
                $self->Debug(
"Job $id (prio: $priority) for run $work->{RUNNUMBER} taken from $hostname queue.\n"
                );

                if ( exists $self->{RunBlacklist}
                    and defined $self->{RunBlacklist} )
                {
                    my %runBlacklistHash =
                      ( @{ $self->{RunBlacklist} },
                        @{ $self->{RunBlacklist} } );
                    ...
                    {
                        $self->Debug( "Dequeued job for blacklisted run "
                              . $work->{RUNNUMBER}
                              . ", discarding it\n" );
                        next;
                    }
